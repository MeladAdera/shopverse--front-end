...
import toast from 'react-hot-toast'; // استيراد مباشر
...
export default function OrdersPage() {
  ...
  const handleCancelOrder = async (orderId: number, orderNumber: string) => {
    ...
    // استخدم toast.custom مباشرة
    toast.custom((t: any) => ( // أضفت النوع any مؤقتاً
      <div className={`${
        t.visible ? 'animate-enter' : 'animate-leave'
      } max-w-md w-full bg-white shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5`}>
        <div className="flex-1 w-0 p-4">
          <div className="flex items-start">
            <div className="shrink-0 pt-0.5"> {/* عدلت هنا */}
                ...
                <div key={order.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                  ...
                  <div className="flex justify-between pt-3 border-t border-gray-100">
                    ...
                    {(order.status === 'pending' || order.status === 'processing') && (
                      <button
                        onClick={() => handleCancelOrder(order.id, order.id.toString())}
